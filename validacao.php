...
Class Validacao{
    ...
    private function unique ($table, $campo, $valor){
        ...
        if ($resultado){
            $this->validacoes []= "O $campo ja está sendo utilizado!";
        }    
    }
    ...
    private function confirmed($campo, $valor, $valorDeConfirmacao){
        if( $valor != $valorDeConfirmacao){
            $this->validacoes[] = "O $campo de confirmação não é igual!";
    ...
    private function min($min, $campo, $valor){
        if(strlen($valor) <= $min){
            $this->validacoes[] = "A $campo precisa ter no mínimo de $min caracteres!";
        }
    }

    private function max($max, $campo, $valor){
        if(strlen($valor) > $max){
            $this->validacoes[] = "A $campo precisa ter no máximo de $max caracteres!";
        }
    }

    private function strong($campo, $valor){
        if(! strpbrk($valor, '*!@#$%&_-?') ){
            $this->validacoes []= "A $campo precisa de um caracter especial (!@#$)";
        }
    }
